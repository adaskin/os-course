...
int sort(struct task *t){
    ...
    if(t[i]->utime <t[j]->utime)
    swap()
}

int update_private_data(struct file *file){
    struct task *t;
    int i = 0;
    for_each_process(t){
        count++;
    }
    struct task *tasks = kmalloc(count*sizeof(struct task*);
    for_each_process(t){
        tasks[count] = t;
        count++;
    ...
    ...
    sort();
    struct my_data *s = file->private;
    for(int i = 0; i < n: i++){
        int len = snprintf(s->buf+i*size, size, "...%d-%d", tasks[i]->utime, stime, )..  
    }
...
ssize_t my_write(struct file *file, const char __user *usr_buf, size_t size, loff_t *offset)
{
    ...
    int a = 0;
    sscanf(buf, "%d", &n);
    printk(KERN_INFO "the value of kernel buf: %s-%d", buf, n);
